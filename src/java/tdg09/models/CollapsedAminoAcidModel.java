...
public class CollapsedAminoAcidModel extends AbstractRateMatrix implements RateMatrix {
    ...
    private double[][] wagRates = getWAGOriginalRates();

    public CollapsedAminoAcidModel(double[] f, int[] aminoAcidList) {
        super(f.length);
        setFrequencies(f);
    ...
    public double[][] getWAGOriginalRates() {
        double[][] rate = new double[19][20];
        // Q matrix
        rate[0][1] = 0.610810;
        rate[0][2] = 0.569079;
        rate[0][3] = 0.821500;
    ...
    protected void rebuildRateMatrix(double[][] rate, double[] parameters) {
        for (int i = 0; i < getFrequencies().length; i++) {
            for (int j = 0; j < getFrequencies().length; j++) {
                if (i < j) {
                    rate[i][j] = wagRates[aminoAcidList[i]][aminoAcidList[j]];
                }
            }
        }
    }

    // We don't need to implement these because we're not using PAL's maximum likelihood estimation
    @Override
